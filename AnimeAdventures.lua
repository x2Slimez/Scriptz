...
--#endregion
    ...
    local autofrmserver = win:Server("หมวดหมู่ออโต้ฟาร์ม", "http://www.roblox.com/asset/?id=11579310982")
    local webhookserver = win:Server("การแจ้งเตือน  ", "http://www.roblox.com/asset/?id=11585480207")
    local diskordserver = win:Server("ดิสคอร์ด   ", "http://www.roblox.com/asset/?id=12155612675")
    local creditsserver = win:Server("เครดิต          ", "http://www.roblox.com/asset/?id=11579371312")


    if game.PlaceId == 8304191830 then

        local unitselecttab = autofrmserver:Channel("👷 เลือกตัวละคร")
        local slectworld = autofrmserver:Channel("🌎 เลือกโลก")
        local devilcity = autofrmserver:Channel("👿 เมืองปีศาจ")
        local autofarmtab = autofrmserver:Channel("🤖 ออโต้ฟาร์ม")
        local autoclngtab = autofrmserver:Channel("🎯 ออโต้ชาเลนจ์")
...
--#region Select Units Tab
        ...
        unitselecttab:Button("เลือกตัวละครที่จะติดตั้ง", function()
            ...
            DiscordLib:Notification("เลือกตัวละครที่จะติดตั้งแล้ว!", "The dropdowns may not show the unit names now, but it will show next time you execute!", "Okay!")

        end)

        local drop = unitselecttab:Dropdown("ตัวละครที่ 1", Units, getgenv().SelectedUnits["U1"], function(bool)
            getgenv().SelectedUnits["U1"] = bool
            Equip()
        end)

        local drop2 = unitselecttab:Dropdown("ตัวละครที่ 2", Units, getgenv().SelectedUnits["U2"], function(bool)
            getgenv().SelectedUnits["U2"] = bool
            Equip()
        end)

        local drop3 = unitselecttab:Dropdown("ตัวละครที่ 3", Units, getgenv().SelectedUnits["U3"], function(bool)
            getgenv().SelectedUnits["U3"] = bool
            Equip()
        end)

        local drop4 = unitselecttab:Dropdown("ตัวละครที่ 4", Units, getgenv().SelectedUnits["U4"], function(bool)
        ...
        if tonumber(axx[2]) >= 20 then
            _G.drop5 = unitselecttab:Dropdown("ตัวละครที่ 5", Units, getgenv().SelectedUnits["U5"], function(bool)
...
--------------// Refresh Unit List \\------------- 
        unitselecttab:Button("รีเซ็ตตัวละคร", function()
            drop:Clear()
            drop2:Clear()
            drop3:Clear()
            drop4:Clear()
...
--#region Select world tab
        ...
        getgenv().diff = slectworld:Dropdown("เลือกระดับความยาก", {"Normal", "Hard"}, getgenv().difficulty, function(diff)
            getgenv().difficulty = diff
            updatejson()
        end)

        local worlddrop = slectworld:Dropdown("เลือกโลก", {"Plannet Namak", "Shiganshinu District", "Snowy Town","Hidden Sand Village", "Marine's Ford",
        "Ghoul City", "Hollow World", "Ant Kingdom", "Magic Town", "Cursed Academy","Clover Kingdom", "Clover Legend - HARD","Hollow Legend - HARD","Cape Canaveral"}, getgenv().world, function(world)
            ...
            getgenv().leveldrop = slectworld:Dropdown("เลือกด่าน", getgenv().levels, getgenv().level, function(level)
            getgenv().level = level
            updatejson()
...
getgenv().portalname = devilcity:Dropdown("เลือกประตู", {"csm_contract_0", "csm_contract_1","csm_contract_2","csm_contract_3","csm_contract_4","csm_contract_5"}, getgenv().portalnameX, function(pornname)
    getgenv().portalnameX = pornname
    updatejson()
end)

devilcity:Button("ซื้อประตู", function()
...
devilcity:Toggle("ออโต้ฟาร์มประตู", getgenv().farmprotal, function(bool)
    getgenv().farmprotal = bool
    updatejson()
end)

devilcity:Label("เล่นได้เฉพาะประตูที่ปลดล็อคเท่านั้น")
devilcity:Label("นอกจากนี้ หากคุณมีประตูระดับต่ำ มันอาจจะเริ่มทำงาน \nดังนั้นอย่าซื้อประตูระดับต่ำที่คุณไม่ต้องการฟาร์ม")

--------------------------------------------------
------------------ Auto Farm Tab -----------------
--------------------------------------------------
--#region Auto Farm Tab
        autofarmtab:Toggle("ออโต้เล่นด่านซ้ำ", getgenv().AutoReplay, function(bool)
            getgenv().AutoReplay = bool
            updatejson()
        end)
        autofarmtab:Toggle("ออโต้ออกด่าน", getgenv().AutoLeave, function(bool)
            getgenv().AutoLeave = bool
            updatejson()
        end)
        autofarmtab:Toggle("ออโต้ฟาร์มอีเว้นท์ ⭐⭐", getgenv().AutoFarmTP, function(bool)
            getgenv().AutoFarmTP = bool
            updatejson()
        end)

        autofarmtab:Toggle("ออโต้ฟาร์มหอคอย", getgenv().AutoFarmIC, function(bool)
            getgenv().AutoFarmIC = bool
            updatejson()
        end)

        autofarmtab:Toggle("ออโต้เล่นเอง", getgenv().AutoFarm, function(bool)
            getgenv().AutoFarm = bool
            updatejson()
        end)

        autofarmtab:Toggle("ออโต้เริ่มด่าน", getgenv().autostart, function(bool)
            getgenv().autostart = bool
            updatejson()
        end)

        autofarmtab:Toggle("ออโต้กดสกิล", getgenv().autoabilities, function(bool)
            getgenv().autoabilities = bool
            updatejson()
        end)

        autofarmtab:Toggle("ออโต้อัพตัวละคร", getgenv().autoupgrade, function(bool)
            getgenv().autoupgrade = bool
            updatejson()
        end)

        autofarmtab:Toggle("ออโต้ขายตัวละคร", getgenv().autosell, function(x)
            getgenv().autosell = x
            updatejson()
        ...
        ---- 
        autofarmtab:Textbox("เลือกเวฟสำหรับการขายตัวละครออโต้ {กดปุ่ม Enter}", tostring(getgenv().sellatwave), false, function(t)
            getgenv().sellatwave = tonumber(t)
            updatejson()
        end)
        
        local autoloadtab = autofrmserver:Channel("⌛ ออโต้รันสคริปต์")
        ...
        autoloadtab:Label("⚠️ หากไม่ทำงานให้วางสคริปต์ในโฟลเดอร์ Autoexec\nfolder!!! ⚠️")

        

		local webhooktab = webhookserver:Channel("🌐 แจ้งเตือน")
		webhooktab:Label("Webhook ส่งการแจ้งเตือนใน Discord ทุกครั้งหลังจบเกม!")
		
		local webhoolPlaceholder
		if getgenv().weburl == "" then
			webhoolPlaceholder = "ใส่ url ที่นี่!"
		else
			webhoolPlaceholder = getgenv().weburl
		end
		webhooktab:Textbox("Webhook URL {กดปุ่ม Enter}" , webhoolPlaceholder, false, function(web_url)
...
--#region Auto Challenge
        autoclngtab:Toggle("ออโต้ชาเลนจ์", getgenv().AutoChallenge, function(bool)
            getgenv().AutoChallenge = bool
            updatejson()
        end)
        local worlddrop = autoclngtab:Dropdown("เลือกชาเลนจ์", {"star_fruit_random","star_remnant","gems", "gold"}, getgenv().selectedreward, function(reward)
            getgenv().selectedreward = reward
            updatejson()
        end)

        autoclngtab:Toggle("ฟาร์มชาเลนจ์ทั้งหมด", getgenv().AutoChallengeAll, function(bool)
...
--#region Auto Buy/Sell
        ...
        local misc = autofrmserver:Channel("💸 ออโต้ ซื้อ/ขาย")
        ...
        misc:Label("ตู้ Special - 2x Mythic")
        misc:Toggle("ออโต้สุ่มตัวละคร {ใช้ตั๋วสุ่ม 1 ใบ}", getgenv().autosummonticketse, function(bool)
        ...
        misc:Toggle("ออโต้สุ่มตัวละคร {สุ่ม 1 ครั้ง}", getgenv().autosummongeme, function(bool)
        ...
        misc:Toggle("ออโต้สุ่มตัวละคร {สุ่ม 10 ครั้ง}", getgenv().autosummongem10e, function(bool)
        ...
        misc:Label("ตู้ Standard")
        misc:Toggle("ออโต้สุ่มตัวละคร {ใช้ตั๋วสุ่ม 1 ใบ}", getgenv().autosummontickets, function(bool)
        ...
        misc:Toggle("ออโต้สุ่มตัวละคร {สุ่ม 1 ครั้ง}", getgenv().autosummongem, function(bool)
        ...
        misc:Toggle("ออโต้สุ่มตัวละคร {สุ่ม 10 ครั้ง}", getgenv().autosummongem10, function(bool)
            getgenv().autosummongem10 = bool
            while getgenv().autosummongem10 do
        ...
        misc:Label("ขายตัวละคร")
        local utts = misc:Dropdown("เลือกระดับความหายาก", {"Rare", "Epic"}, getgenv().UnitToSell, function(u)
            getgenv().UnitToSell = u
        end)

        misc:Toggle("ออโต้ขายตัวละคร", getgenv().UnitSellTog, function(bool)
...
--#region --- Inside match ---
    ...
    local autoclngtab = autofrmserver:Channel("🎯 ออโต้ชาเลนจ์")
    local autoloadtab = autofrmserver:Channel("⌛ ออโต้รันสคริปต์")
    local autoseltab = autofrmserver:Channel("💸 ออโต้ขาย")
    local webhooktab = webhookserver:Channel("🌐 แจ้งเตือน")
...
--#region Auto Farm Tab
        ...
        --// Set Position \\--
        autofarmtab:Button("เซ็ตตำแหน่งตัวละคร 1", function()
            DiscordLib:Notification("กำหนดการวางตัวละคร 1",
                "คลิกบนพื้นเพื่อกำหนดตำแหน่งตัวละคร\n (อย่ากด \"เสร็จสิ้น\" จนกว่าคุณจะกำหนดตำแหน่ง)",
                "เสร็จแล้ว")
                warn(1)
            MouseClick("UP1")
            warn(2)
        ...
        autofarmtab:Button("เซ็ตตำแหน่งตัวละคร 2", function()
            DiscordLib:Notification("กำหนดการวางตัวละคร 2",
                "คลิกบนพื้นเพื่อกำหนดตำแหน่งตัวละคร\n (อย่ากด \"เสร็จสิ้น\" จนกว่าคุณจะกำหนดตำแหน่ง)",
                "เสร็จแล้ว")
            MouseClick("UP2")
        end)
        autofarmtab:Button("เซ็ตตำแหน่งตัวละคร 3", function()
            DiscordLib:Notification("กำหนดการวางตัวละคร 3",
                "คลิกบนพื้นเพื่อกำหนดตำแหน่งตัวละคร\n (อย่ากด \"เสร็จสิ้น\" จนกว่าคุณจะกำหนดตำแหน่ง)",
                "เสร็จแล้ว")
            MouseClick("UP3")
        end)
        autofarmtab:Button("เซ็ตตำแหน่งตัวละคร 4", function()
            DiscordLib:Notification("กำหนดการวางตัวละคร 4",
                "คลิกบนพื้นเพื่อกำหนดตำแหน่งตัวละคร\n (อย่ากด \"เสร็จสิ้น\" จนกว่าคุณจะกำหนดตำแหน่ง)",
                "เสร็จแล้ว")
            MouseClick("UP4")
        end)
        ...
        if tonumber(axxc[2]) >= 20 then
            autofarmtab:Button("เซ็ตตำแหน่งตัวละคร 5", function()
                DiscordLib:Notification("กำหนดการวางตัวละคร 5",
                    "คลิกบนพื้นเพื่อกำหนดตำแหน่งตัวละคร\n (อย่ากด \"เสร็จสิ้น\" จนกว่าคุณจะกำหนดตำแหน่ง)",
                    "เสร็จแล้ว")
                MouseClick("UP5")
            end)
        ...
        if tonumber(axxc[2]) >= 50 then
            autofarmtab:Button("เซ็ตตำแหน่งตัวละคร 6", function()
                DiscordLib:Notification("กำหนดการวางตัวละคร 6",
                    "คลิกบนพื้นเพื่อกำหนดตำแหน่งตัวละคร\n (อย่ากด \"เสร็จสิ้น\" จนกว่าคุณจะกำหนดตำแหน่ง)",
                    "เสร็จแล้ว")
                MouseClick("UP6")
            end)
        ...
        autofarmtab:Label("--- การกำหนดค่าที่บันทึกไว้ (ไม่รีเซ็ต) ---")
        autofarmtab:Label("ออโต้ขายตัวละครที่เวฟ: " .. tostring(getgenv().sellatwave))
        autofarmtab:Label("Webhook: " .. tostring(getgenv().weburl))
        autofarmtab:Label("ออโต้ฟาร์ม: " .. tostring(getgenv().AutoFarm))
        autofarmtab:Label("ออโต้เริ่มด่าน: " .. tostring(getgenv().autostart))
        autofarmtab:Label("ออโต้ขาย: " .. tostring(getgenv().autosell))
        autofarmtab:Label("ออโต้อัพตัวละคร: " .. tostring(getgenv().autoupgrade))
        autofarmtab:Label("ระดับความยาก: " .. tostring(getgenv().difficulty))
        autofarmtab:Label("โลกที่เลือก: " .. tostring(getgenv().world))
        autofarmtab:Label("ด่านที่เลือก: " .. tostring(getgenv().level))
        autofarmtab:Label(" ")
        autofarmtab:Label(" ")
...
--#region Auto Sell Tab
        autoseltab:Toggle("ออโต้ขายตัวละคร", getgenv().autosell, function(x)
            getgenv().autosell = x
            updatejson()
            if getgenv().autosell == false then
                getgenv().disableatuofarm = false
        ...
        autoseltab:Textbox("เลือกเวฟสำหรับการขายตัวละครออโต้ {กดปุ่ม Enter}", getgenv().sellatwave, false, function(t)
            getgenv().sellatwave = tonumber(t)
            updatejson()
        end)
--#endregion
...
--#region Webhook
        ...
        webhooktab:Button("ทดลองการแจ้งเตือน", function()
            webhook()
        end)
--#endregion
...
gettrigon:Label("❗ เข้ามาพูดคุยกันได้ที่ Discord NekoHub")
gettrigon:Label("❗ คลิกปุ่มด้านล่างเพื่อคัดลอก Discord ของ NekoHub")
...
--#region Credits
local credits = creditsserver:Channel("✨ เครดิต")
credits:Label("x2Skyz#4852")
credits:Button("👉 คัดลอกลิงก์ Discord อย่างเป็นทางการ!", function()
    setclipboard("https://discord.gg/gBrJpjdTVh")
    DiscordLib:Notification("คัดลอกสำเร็จแล้ว✅", "✅ คัดลอกลิงก์เชิญ Discord ของ Neko Hub ไปยังคลิปบอร์ดของคุณแล้ว", "เสร็จสิ้น✅")
end)
--#endregion
